	...
	function recommend_action()
	{
		...
		if($pid){
			...
			if($s==1){
				...
				$this->automsg('管理员设置：推荐兼职职位《'.$info['name'].'》'.$addday.'天',$info['uid']);
			}else{
		...
		if(!empty($codearr)){
			if($s==1){
                ...
                if(is_array($list)){
					...
					if($list){
						foreach($list as $v){
							$this->automsg('管理员设置：推荐兼职职位《'.$v['name'].'》'.$addday.'天',$v['uid']);
						}
					}
		...
		if(is_array($id)){
			...
			foreach($rows as $value){
				if($value['edate']!=0){
					...
					$this->automsg('管理员操作：延期兼职职位《'.$value['name'].'》'.intval($endtime).'天',$value['uid']);
				}
			}
