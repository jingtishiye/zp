...
class invite_controller extends company{
	function index_action(){
		if(trim($_GET['keyword'])){
			$resume=$this->obj->DB_select_alls("resume","resume_expect","a.uid=b.uid and a.`r_status`<>'2' and a.`name` like '%".trim($_GET['keyword'])."%' and a.`def_job`=b.`id`","a.`name`,a.`uid`,a.`sex`,a.`edu`,a.`nametype`,b.`job_classid`,a.`exp`,b.`salary`"); 
			if(is_array($resume)){
				foreach($resume as $v){
					$uidarr[]=$v['uid'];
				}
